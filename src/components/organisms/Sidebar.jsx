...
const Sidebar = ({ isOpen, onClose, className }) => {
        ...
        <NavLink
            ...
            className={({ isActive }) =>
                cn(
                    ...
                    isActive ? 'bg-gradient-to-r from-primary-100 to-secondary-100 text-primary-700 shadow-sm' : 'text-slate-600 hover:text-slate-800'
                )
            }
        >
            ...
            {count > 0 && (
                <span
                    className={cn(
                        ...
                        isCategory ? 'bg-slate-200 text-slate-600' : 'bg-primary-100 text-primary-600'
                    )}
                >
                    {count}
                </span>
    ...
    // Desktop sidebar
    return (
        <aside className={cn('hidden lg:block w-64 bg-white border-r border-slate-200 h-full', className)}>
            <SidebarContent
                navItems={navItems}
                categories={categories}
                taskCounts={taskCounts}
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="p-6 border-b border-slate-200">
            <div className="flex items-center gap-2">
                ...
                <span className="text-xl font-bold bg-gradient-to-r from-primary-600 to-secondary-600 bg-clip-text text-transparent">TaskFlow</span>
            </div>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-6 overflow-y-auto">
            ...
            <div className="space-y-1">
                {navItems.map((item) => (
                    <NavItem key={item.href} href={item.href} icon={item.icon} label={item.label} count={item.count} />
                ))}
            </div>

            {/* Categories */}
            <div className="space-y-3">
                <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wide px-3">Categories</h3>
                <div className="space-y-1">
                    {loading ? (
            ...
            <div className="border-t border-slate-200 pt-4">
                <NavItem href="/completed" icon="CheckCheck" label="Completed" count={taskCounts.completed || 0} />
            </div>
        </nav>
    </div>
);
