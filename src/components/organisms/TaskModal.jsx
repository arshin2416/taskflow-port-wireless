...
const TaskModal = ({ isOpen, onClose, task = null, onSave, className }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let savedTask;
            // Prepare task data with properly formatted files
            const taskData = {
                ...formData,
        ...
        <AnimatePresence>
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                <motion.div
                    ...
                    className={cn('bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto', className)}
                >
                    {/* Header */}
                    <div className="flex items-center justify-between p-6 border-b border-slate-200">
                        <h2 className="text-xl font-semibold text-slate-800">{task ? 'Edit Task' : 'Add New Task'}</h2>
                        <button
                            onClick={onClose}
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <FormField label="Priority" type="select" value={formData.priority} onChange={handleChange('priority')}>
                        ...
                        <FormField label="Due Date" type="date" value={formData.dueDate} onChange={handleChange('dueDate')} />

                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-2">Files</label>
                            <FileUploader value={formData.files} onChange={handleChange('files')} error={errors.files} />
                        </div>
                        {/* Actions */}
