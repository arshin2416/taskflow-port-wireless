...
const TaskList = ({
    ...
    if (error) {
        return <ErrorView error={error} className={className} />;
    }

    if (sortedTasks.length === 0) {
        return <Empty title={emptyTitle} description={emptyDescription} onAction={onAddTask} actionLabel="Add Task" className={className} />;
    }
            ...
            <div className="space-y-3">
                <AnimatePresence mode="popLayout">
                    {sortedTasks.map((task) => (
                        <TaskCard key={task.Id} task={task} onToggleComplete={onTaskToggle} onEdit={onTaskEdit} onDelete={onTaskDelete} />
                    ))}
                </AnimatePresence>
            </div>
        </div>
